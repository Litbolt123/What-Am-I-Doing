...
- **2026-07-19** — **Maple Bear platform checklist + master prompt:** Cross-app docs from Hub + YouTube Downloader + WAID → `GitHub projects/docs/maple-bear-app-platform-checklist.md` + `maple-bear-app-platform-master-prompt.md`. WAID contributed: crash logger / support bundle, Catch-up onboarding, PIN gate, SQLite settings, richest Inno update UX.
- **2026-07-20** — **Ship v1.2.4:** Rules editor bottom pane scroll fix ([`RulesWindow.xaml`](src/WhatAmIDoing/RulesWindow.xaml)). Tag **`v1.2.4`** → Windows installer workflow.
- **2026-07-20** — **Rules editor scroll fix:** Bottom pane (`Match type and pattern`, Advanced, etc.) used `Grid.Row` **Auto** so `ScrollViewer` grew with content and never scrolled; row is now **\*** height (min 220px) so the editor scrolls within the window ([`RulesWindow.xaml`](src/WhatAmIDoing/RulesWindow.xaml)).
- **2026-07-08** — **Ship v1.2.3:** HTML export — list every YouTube video (setting, default on); new transparent clock + magnifier icon (procedural [`make-icon.ps1`](scripts/make-icon.ps1)). Tag **`v1.2.3`** → Windows installer workflow.
- **2026-07-08** — **App icon — transparent outside clock:** [`scripts/make-icon.ps1`](scripts/make-icon.ps1) now **renders** clock + magnifier by default — blue-to-teal **fill inside the dial only**, white hands/ticks/glass, **transparent outside** (no white square corners). Regenerated [`assets/app-icon-source.png`](assets/app-icon-source.png) + [`src/WhatAmIDoing/app.ico`](src/WhatAmIDoing/app.ico) (taskbar, tray, installer). Use `-FromPng` for custom PNG sources.
- **2026-07-06** — **HTML export — full YouTube list:** Setting **List every YouTube video in HTML export** (Dashboard section, default **on**, `html_export_youtube_full_list`) via [`HtmlExportSettingsService`](src/WhatAmIDoing/Services/HtmlExportSettingsService.cs). When on, [`HtmlReportExporter.AppendWebsitesAtAGlance`](src/WhatAmIDoing/Services/HtmlReportExporter.cs) lists **all** YouTube/YouTube Music titles with engaged time (no top-6 cap, full titles); skips duplicate “Top YouTube” section. When off, prior top-6 + top-15 behavior. Parent walkthrough for settings deferred.
...
- **2026-05-19** — **Update tray notify + start in tray:** Fixed update balloon suppressed across reboots (`update_last_notified_version` no longer blocks forever — once per session until upgraded); update check runs before optional one-time tray-icon hint. **Settings:** `start_in_system_tray` ([`StartupTrayService`](src/WhatAmIDoing/Services/StartupTrayService.cs)).
- **2026-05-19** — **Ship v1.2.1:** Theme polish on dialogs; Rules Apps/Sites tabs + capture extras + open-window picker; HTML rule notes; chart legend polish + Time/%/Both toggle; scroll ghost metrics fix; split Refresh hover. Tag **`v1.2.1`** → Windows installer workflow.
- **2026-05-19** — **Legend display toggle:** Small **Legend** combo on chart card — **Time** / **%** / **Both** ([`ChartLegendDisplayService`](src/WhatAmIDoing/Services/ChartLegendDisplayService.cs), setting `dashboard_legend_display`); legend % = engaged share; **By category** table **%** = total range. HTML export uses same legend mode.
- **2026-05-19** — **Chart legend polish (rev 2):** Legend default **time only**; **%** column on **By category** table. [`ChartLegendHelper`](src/WhatAmIDoing/Services/ChartLegendHelper.cs); swatches 18px dashboard / 14px HTML.
- **2026-05-19** — **HTML rule notes + scroll ghost metrics fix:** [`HtmlReportExporter.AppendRuleNotesSection`](src/WhatAmIDoing/Services/HtmlReportExporter.cs) — **only rules with non-empty `notes`** in table “How categories are classified” (Category / Pattern / Match / Note); [`MatchKindUi.GetMatchLabel`](src/WhatAmIDoing/Services/MatchKindUi.cs). **Dashboard scroll bug:** metric strip was in `Grid.Row="3"` with `Height="*"` while report `ScrollViewer` was row 4 → transparent scroll gaps showed metrics behind; fixed with **Auto** row for metrics + **\*** for scroll + opaque `DashBgBrush` on scroll viewer ([`MainWindow.xaml`](src/WhatAmIDoing/MainWindow.xaml)).
- **2026-05-19** — **Theme polish + Rules Apps/Sites + capture extras (local, no release):** [`DashboardUi.EnsureTheme`](src/WhatAmIDoing/Services/DashboardUi.cs) on About, Pin, WebsiteRuleDialog, DetectionTune, FirstRunChecklist, CategoryMerge/Drill, TrackingHelp, OpenWindowPicker; dialogs use `DashBgBrush` / card styles ([`TrackingHelpWindow.xaml`](src/WhatAmIDoing/TrackingHelpWindow.xaml) last hardcoded bg fixed). **Rules:** filter tabs **All | Apps | Sites & pages** via [`MatchKindUi`](src/WhatAmIDoing/Services/MatchKindUi.cs); capture **3/5/8 s** wait; **Pick from open windows…** → [`OpenWindowPickerWindow`](src/WhatAmIDoing/OpenWindowPickerWindow.xaml) + [`ForegroundWindowHelper.EnumerateVisibleWindows`](src/WhatAmIDoing/Services/ForegroundWindowHelper.cs) (build fix: lambda `_` param shadowed discard on `GetWindowThreadProcessId`). **Deferred:** **1.2.1** ship; **HTML export rule notes** + **chart legend polish** (user asked plan first). **Backlog plan:** HTML section “How categories are classified” from `rules.notes`; legend — bigger swatches + duration (and optional %) in dashboard + HTML `BuildLegendHtml`.
- **2026-05-18** — **Ship v1.2.0:** Dashboard redesign, Catch up tour, tracking status Summary, continuous refresh, Settings/Rules UX + help tips, `AppSettingsDefaults`, installer version-on-update. Tag **`v1.2.0`** pushed → Windows installer workflow / GitHub Release.
- **2026-05-18** — **Maintainer Settings defaults + Rules layout:** [`AppSettingsDefaults.cs`](src/WhatAmIDoing/Data/AppSettingsDefaults.cs) — 1 min idle, 1.5 min thinking, **1 sec** sample, **20×** YouTube, detection on, watchdog + desktop shortcut + high contrast + keyboard helpers + backup reminder on; fresh install + one-time migration from 2s/10×. **Rules:** table row `*` fills window (fixed DockPanel bug), wider Match column + ellipsis/tooltip, editor scrolls, match section collapsed until capture/edit.
- **2026-05-18** — **Fix Settings/Rules won't open:** `HelpTipButton` used `StaticResource DashHelpButton` before parent window resources existed → `DashboardUi.EnsureTheme(FrameworkElement)` + merge on control before `InitializeComponent`. Settings open path logs to crash log on failure (like Rules).
- **2026-05-18** — **Simple Settings & Rules UX:** [`HelpTipButton`](src/WhatAmIDoing/Controls/HelpTipButton.xaml) — hover tooltip + click for longer help; [`DashHelpButton`](src/WhatAmIDoing/Themes/DashboardTheme.xaml) / `DashSimpleCheckBox` / `DashSectionExpander`. **Settings** — short checkboxes and numeric rows; rare sections in expanders (screenshots, chart colors, backup); removed wall-of-text paragraphs. **Rules** — slimmer grid (Match/Pattern/Category/Pri/Skip totals); capture-first editor; match/advanced/more-actions in expanders.
- **2026-05-18** — **Rules + Settings UI sync with dashboard:** [`DashboardUi.EnsureTheme`](src/WhatAmIDoing/Services/DashboardUi.cs) merges [`DashboardTheme.xaml`](src/WhatAmIDoing/Themes/DashboardTheme.xaml) in Main/Rules/Settings constructors; new **`DashDataGrid`** / **`DashHintText`** styles. [`RulesWindow`](src/WhatAmIDoing/RulesWindow.xaml) — card layout, themed buttons, default **960×700** (min **860**), **Pattern** column `Width="2*" MinWidth="200"`, resizable columns, horizontal scroll; editor in bottom card. [`SettingsWindow`](src/WhatAmIDoing/SettingsWindow.xaml) — same palette/cards/section titles/toolbar buttons; **Save/Cancel/About** pinned in footer card outside scroll.
- **2026-05-06** — **Continuous dashboard refresh:** Split **Refresh** control with **▾** menu — checkbox **Continuous refresh while dashboard is open** (`dashboard_continuous_refresh`, 30s interval, pauses when window hidden to tray). [`DashboardRefreshService.cs`](../src/WhatAmIDoing/Services/DashboardRefreshService.cs).
- **2026-05-06** — **Parent tracking status (Summary):** [`TrackingStatusSummary.cs`](../src/WhatAmIDoing/Services/TrackingStatusSummary.cs) — always-visible box in Summary: install baseline, **Installed on this PC — date/time** when first run falls in range, **App updated**, **Not tracking** (closed→resumed), **Stopped to install update**; blue border when anything notable. **More details** keeps voice/quiet/ignored + full chronological log. Requires **lifecycle logging** on (Settings → Family controls, default on).
- **2026-05-06** — **Summary card cleanup:** Dashboard **Summary** shows only **where you spent time online** + **Compared to the previous period**; removed per-view rules/tip/install-id noise. **More details** expander (collapsed, hidden when empty) holds voice/mic, ignored %, quiet hours, app activity log. Install id moved to **About**; HTML export unchanged. Walkthrough copy updated for Summary step.
- **2026-05-06** — **In-dashboard Catch up (replaces overlay tour):** Removed topmost [`DashboardTutorialOverlay`](src/WhatAmIDoing/DashboardTutorialOverlay.xaml) (stayed on screen when app unfocused). New **`CatchUpCard`** on dashboard: **walkthrough** (Next/Skip, in-window blue border highlight + scroll), then **What’s new in vX** (`dashboard_whats_new_seen_version`), then persistent **All caught up! / Nothing new here**. **Catch up** header button + Settings replay. Prior: `_` mnemonics removed; **Tracking** in **`?`** / Settings; summary simplified.
- **2026-05-06** — **Dashboard redesign + category table gaps:** **By category** huge empty rows fixed — shared `Grid` row height matched tall **Summary** column; now **independent left/right column panels** (`ReportDetailsLeftPanel` / `ReportDetailsRightPanel`) with reparenting in narrow mode; **`FitDataGridToRows`** sizes DataGrids to row count. **UI redesign v1:** [`Themes/DashboardTheme.xaml`](../src/WhatAmIDoing/Themes/DashboardTheme.xaml) (palette, cards, toolbar/DataGrid styles); [`MainWindow.xaml`](../src/WhatAmIDoing/MainWindow.xaml) — header, metric strip (on computer / active / thinking / idle), restyled toolbar + chart + detail cards; category/process use **StackPanel** not stretch **DockPanel**.
- **2026-05-06** — **Shareable agent doc:** Added [`docs/github-versioning-releases-agent-guide.md`](github-versioning-releases-agent-guide.md) — repo-agnostic playbook for MSBuild `Version`, tags `v*`, GitHub Actions, Releases vs artifacts, tag/version verification, `softprops/action-gh-release`, prerelease/`make_latest`, common CI failures; points to this repo as reference + [`docs/releasing.md`](releasing.md) for product-specific steps.
- **2026-05-06** — **User wrap-up:** Broader **UI redesign** deferred to a future session; tonight’s work stopped after narrow-window dashboard fixes (see previous log line).
- **2026-05-06** — **Narrow-window dashboard polish:** [`MainWindow.xaml`](src/WhatAmIDoing/MainWindow.xaml) / [`MainWindow.xaml.cs`](src/WhatAmIDoing/MainWindow.xaml.cs) — **`ApplyReportDetailsLayoutForWidth`** (breakpoint ~920px on `ReportDetailsRoot.ActualWidth`, skips until layout > 10px) toggles **single-column** vs **two-column** report details (`ReportDetailsRightColumn` width 0 vs `*`); **`RefreshChartForViewportWidth`** + **`SyncChartCanvasWidthToHost`** bind chart canvas width to **`ChartAreaScrollViewer`** so the hourly chart does not force horizontal scroll; **`ChartAreaScrollViewer_OnSizeChanged`** (width) + window width resize + **`DispatcherPriority.Loaded`** pass after first paint; chart **`ClipToBounds="True"`**; removed canvas **`SizeChanged` → full `UpdateChart`** loop. Release build verified to alternate `-o` path when `WhatAmIDoing.exe` is running (file lock on default `bin\Release`).
- **2026-05-12** — **Ship v1.1.5:** [`Directory.Build.props`](Directory.Build.props) / [`app.manifest`](src/WhatAmIDoing/app.manifest) / Inno fallback → **1.1.5**; [`docs/RELEASE_BODY.md`](docs/RELEASE_BODY.md) — version-only bump so installs on **1.1.4** can verify the updater again. Annotated tag **`v1.1.5`** pushed → Windows installer workflow.
- **2026-05-12** — **Main report wheel over Summary / grids:** [`MainWindow`](src/WhatAmIDoing/MainWindow.xaml) — `PreviewMouseWheel` bubbles to **`MainReportScrollViewer`** when nested **chart** / **DataGrid** scroll can’t consume the wheel (or at inner scroll ends); chrome **Border** handlers for Summary / Highlights / category / process headers so wheel works over padding and tab chrome ([`MainWindow.xaml.cs`](src/WhatAmIDoing/MainWindow.xaml.cs)).
- **2026-05-12** — **Dashboard layout (month / long ranges):** [`MainWindow.xaml`](src/WhatAmIDoing/MainWindow.xaml) — single outer **`ScrollViewer`** for chart + Summary / Highlights / grids so tall stacked-day charts no longer starve lower panels on short laptops; chart area **`MaxHeight`** scales with window ([`UpdateChartScrollMaxHeight`](src/WhatAmIDoing/MainWindow.xaml.cs)); **`RowDefinition`** `*` → **`Auto`** for detail rows; **`MinHeight`** on panels; **`DataGrid`** **`VerticalScrollBarVisibility`** + **`MaxHeight`** on highlights/category/process. **`MinWidth`** 820→**760**, **`MinHeight`** 540→**520**.
- **2026-05-12** — **GitHub repo description typo:** Public API showed **Description** `youo` → **you** (link previews / unfurls). Not stored in git; maintainer must edit **Settings → General → Description** on GitHub. README [**GitHub Releases** section](README.md#github-releases-for-people-who-only-download) now has copy-paste canonical one-liner.
- **2026-05-12** — **Inno: show existing version on update:** [`installer/WhatAmIDoing.iss`](installer/WhatAmIDoing.iss) — `[Code]` reads uninstall **DisplayVersion** / **InstallLocation** (`HKCU` then `HKLM64`/`HKLM`), falls back to **`GetVersionNumbers`** on `%LocalAppData%\Programs\WhatAmIDoing\WhatAmIDoing.exe`**; **`UpdateReadyMemo`** + wizard **caption** show current → **`{#AppVersion}`**. [`docs/releasing.md`](docs/releasing.md) table note: keep **`APP_UNINSTALL_SUBKEY`** in sync if **`AppId`** changes.
- **2026-05-12** — **CI fix (v1.1.4 retag):** [`SettingsWindow.xaml.cs`](src/WhatAmIDoing/SettingsWindow.xaml.cs) — `System.Windows.Input.Mouse.OverrideCursor` (unqualified `Mouse` failed compile on GitHub Actions). **`v1.1.4`** tag moved to the fix commit so the Windows installer workflow can succeed.
- **2026-05-12** — **Ship v1.1.4:** [`Directory.Build.props`](Directory.Build.props) / [`app.manifest`](src/WhatAmIDoing/app.manifest) / Inno fallback → **1.1.4**; [`docs/RELEASE_BODY.md`](docs/RELEASE_BODY.md) — GitHub **paginated release scan** + **Settings**-first update UX. Annotated tag **`v1.1.4`** pushed → Windows installer workflow / GitHub Release. User plans **1.1.5** after sibling installs **1.1.4** for another updater pass.
- **2026-05-12** — **Update check accuracy + Settings UX:** [`UpdateCheckService.CheckLatestReleaseAsync`](src/WhatAmIDoing/Services/UpdateCheckService.cs) lists published releases (paginated) and picks the **max** parseable tag instead of **`/releases/latest`** (fixes wrong “up to date” when GitHub’s Latest pointer or pre-release flags disagree). **Check for updates** + **Download and run installer** moved from [`AboutWindow`](src/WhatAmIDoing/AboutWindow.xaml) to [`SettingsWindow`](src/WhatAmIDoing/SettingsWindow.xaml) under **Updates (GitHub)**; About points users to Settings. Docs: [`docs/updating-app.md`](docs/updating-app.md), [`docs/updater-test-two-releases.md`](docs/updater-test-two-releases.md).
- **2026-05-06** — **Release prep v1.1.2 + v1.1.3:** Two commits on `main`: full **1.1.2** ship ([`Directory.Build.props`](Directory.Build.props), [`app.manifest`](src/WhatAmIDoing/app.manifest), [`installer/WhatAmIDoing.iss`](installer/WhatAmIDoing.iss), [`docs/RELEASE_BODY.md`](docs/RELEASE_BODY.md)) with updates/tune/idle/YouTube work; then **1.1.3** version-only bump + [`docs/updater-test-two-releases.md`](docs/updater-test-two-releases.md) so the user can tag **`v1.1.2`** on the first commit and **`v1.1.3`** on HEAD, push both tags, install **1.1.2** from its release asset, and verify the in-app updater against **1.1.3**.
- **2026-05-12** — **Update check every launch:** Removed **20h** throttle on startup GitHub check; delay **50s→12s**; Settings + docs copy = **each session** when auto-check on ([`App.xaml.cs`](src/WhatAmIDoing/App.xaml.cs), [`UpdateCheckService`](src/WhatAmIDoing/Services/UpdateCheckService.cs) constant remark, [`SettingsWindow.xaml`](src/WhatAmIDoing/SettingsWindow.xaml), [`docs/updating-app.md`](docs/updating-app.md)). [`AboutWindow`](src/WhatAmIDoing/AboutWindow.xaml) confirmation + copy stress **full exit (not tray)**; lifecycle kind **`quit_update`** + detail with this vs GitHub release version; [`App.OnExit`](src/WhatAmIDoing/App.xaml.cs) skips generic **`quit`** when `_installerUpgradeExit`; [`MainWindow.BuildLifecycleSummaryText`](src/WhatAmIDoing/MainWindow.xaml.cs) label **Stopped (installing app update)**.
- **2026-05-12** — **About: download + run installer:** [`UpdateCheckService.DownloadInstallerToTempAsync`](src/WhatAmIDoing/Services/UpdateCheckService.cs) (long-timeout HTTP); [`AboutWindow`](src/WhatAmIDoing/AboutWindow.xaml) **Download and run installer…** after check; [`App.ExitForInstallerUpgrade`](src/WhatAmIDoing/App.xaml.cs) + [`MainWindow`](src/WhatAmIDoing/MainWindow.xaml.cs) `BypassMainWindowCloseCancel` so shutdown is not blocked by tray-hide **Closing** handler; [`docs/updating-app.md`](docs/updating-app.md).
- **2026-05-12** — **Automatic update check (GitHub):** [`UpdateCheckService`](src/WhatAmIDoing/Services/UpdateCheckService.cs) parses **`WhatAmIDoing-Setup-*.exe`** `browser_download_url`; [`App`](src/WhatAmIDoing/App.xaml.cs) **each-session** background check (~12s after start) + tray balloon + click opens browser (no silent install); Settings toggles [`auto_check_updates`](src/WhatAmIDoing/Services/UpdateCheckService.cs) / [`update_notify_tray`](src/WhatAmIDoing/Services/UpdateCheckService.cs); [`AboutWindow`](src/WhatAmIDoing/AboutWindow.xaml.cs) manual check prefers direct installer URL; [`docs/updating-app.md`](docs/updating-app.md); support bundle keys.
- **2026-05-12** — **Tune detection custom window:** [`DetectionTuneWindow`](src/WhatAmIDoing/DetectionTuneWindow.xaml) — time range preset **Custom (estimate — back from now)** + duration + minutes/hours; clamps **1 min … 48 h**; [`GetRangeUtc`](src/WhatAmIDoing/DetectionTuneWindow.xaml.cs) / Analyze validation + process list reload on unit change / duration lost focus.
- **2026-05-06** — **Tune detection NRE:** `RangeCombo` default selection fired `SelectionChanged` during `InitializeComponent` before `ProcessCombo` existed → `EditableComboHelper.GetText(null)`. Guard `RangeCombo` / `ProcessCombo` handlers with `IsLoaded` + null checks; [`EditableComboHelper.GetText`](src/WhatAmIDoing/Services/EditableComboHelper.cs) accepts null `ComboBox`.
- **2026-05-06** — **Tune detection UX:** [`DetectionTuneWindow`](src/WhatAmIDoing/DetectionTuneWindow.xaml.cs) reads the process field via [`EditableComboHelper`](src/WhatAmIDoing/Services/EditableComboHelper.cs) (editable `ComboBox` text was often empty until LostFocus — Analyze / Apply looked like no-ops). [`MainWindow`](src/WhatAmIDoing/MainWindow.xaml.cs) try/finally + error `MessageBox` around opening Tune; **ShowInTaskbar** on Tune + [`PinPromptWindow`](src/WhatAmIDoing/PinPromptWindow.xaml) so dialogs are reachable if stacked behind other windows.
- **2026-05-06** — **YouTube Highlights accuracy:** [`TitleContextExtractor`](src/WhatAmIDoing/Services/TitleContextExtractor.cs) — fixed **tail validation** after `… - YouTube` (removed erroneous `.TrimStart()` on the remainder so `Video - YouTube - Chrome` matches again); added **` · `** tab separator; **`msedgewebview2`** / **`chrome_proxy`** to browser process list; **`YouTube`** (store/PWA-style exe name) uses the same browser title parsing.
- **2026-05-06** — **Desktop shortcut option:** [`DesktopShortcutService`](src/WhatAmIDoing/Services/DesktopShortcutService.cs) (WScript `.lnk`); Settings ▸ Family controls checkbox + `desktop_shortcut` setting; support bundle key.
- **2026-05-06** — **Idle / detection upgrades:** XInput **gamepad** polling ([`GameControllerIdleTracker`](src/WhatAmIDoing/Services/GameControllerIdleTracker.cs) + [`IdleHelper.GetCombinedIdleTime`](src/WhatAmIDoing/Services/IdleHelper.cs)); passive audio **peak fallback** + [`ForegroundAppRenderEngagement`](src/WhatAmIDoing/Services/AudioSessionInspector.cs) for HDMI/TV; Settings toggles; **YouTube Music** titles ([`TitleContextExtractor`](src/WhatAmIDoing/Services/TitleContextExtractor.cs)); **Tune detection** dialog ([`DetectionTuneWindow`](src/WhatAmIDoing/DetectionTuneWindow.xaml)) + [`DetectionTuneAnalyzer`](src/WhatAmIDoing/Services/DetectionTuneAnalyzer.cs) / [`TuningHintsStore`](src/WhatAmIDoing/Services/TuningHintsStore.cs) hints in `%LocalAppData%\WhatAmIDoing\tuning_hints.json`; [`GetDistinctProcessNamesBetween`](src/WhatAmIDoing/Data/AppDatabase.cs).
- **2026-05-06** — **User feedback (idle accuracy):** Explained **Wii Remote / Dolphin** — Windows `GetLastInputInfo` ignores non-mouse/keyboard input; mitigation = passive **speaker** engagement + Rules idle override for `Dolphin`; future = optional gamepad/XInput/Raw Input as activity. **YouTube** — passive audio + YouTube title scale; failures from mute, title parse (incl. YouTube Music), or audio session not Active.
- **2026-05-06** — **Git:** Finished interrupted `git pull --rebase` (resolved `context-summary.md`, `parents.md`, `AppDatabase.cs` conflicts — kept family-monitor + seed colors). **`main`** pushed; annotated **`v1.1.1`** pushed → Windows installer workflow / GitHub Release.
