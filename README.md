...
3. Run the file and follow the wizard: on **first install**, **notice** (liability / data) and **license** (terms); on **update**, those are skipped unless terms changed. Then choose **Express** or **Advanced** install. **Advanced** can optionally turn on stricter idle defaults and/or enable screen captures for the first run (you can change everything later in Settings). The setup **does not** install a separate Microsoft .NET download — the app EXE already includes the **.NET 8** stack it needs (self-contained build).

It installs **for your user only** (no administrator prompt) into `%LocalAppData%\Programs\WhatAmIDoing`.

After install, start **What Am I Doing** from the Start menu; it can also start in the tray. Your activity data is stored separately under `%LocalAppData%\WhatAmIDoing\`. **Updates** over an existing install skip the liability notice and license pages unless the published terms version changed (you accept once per terms version).
...
**Crashes immediately with no data folder?** If **Event Viewer → Application** shows **WhatAmIDoing.exe** faulting in **KERNELBASE.dll** with **`Exception code 0xc000041d`** (often **Event 1000**, *Application Error*), that is a fatal callback failure often seen with **compressed** single-file bundles on some machines. A follow-up **Event 1001** from **Windows Error Reporting** with the same **Report Id** is just the bundled crash report for the same exit—not a second bug. Install **1.0.2.5** or newer from Releases (installer uses an **uncompressed** single-file publish), or ask a developer to rebuild with `EnableCompressionInSingleFile=false`.
...
The generated setup EXE is under **`installer\Output\`** (for example `WhatAmIDoing-Setup-1.0.2.5.exe`). That folder is gitignored.
