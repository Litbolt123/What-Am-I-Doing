...
**Crashes immediately with no data folder?** If **Event Viewer → Application** shows **WhatAmIDoing.exe** faulting in **KERNELBASE.dll** with **`Exception code 0xc000041d`**, that is a fatal callback failure often seen with **compressed** single-file bundles on some machines. Install **1.0.2.5** or newer from Releases (installer uses an **uncompressed** single-file publish), or ask a developer to rebuild with `EnableCompressionInSingleFile=false`.
